...
impl Scenario {
    async fn boot(mock: MockGateway) -> Self {
        ...
        //
        // Send it only AFTER raw mode: while the child is still in the pty's
        // default canonical termios, Ctrl+O is VDISCARD on macOS/BSD ttys —
...
// ---------------------------------------------------------------------------
// TEMPORARY macOS diagnostic — delete after the VDISCARD/FLUSHO verdict.
// Twins against one mock: A gets no key at all, B gets the exact
// Scenario::boot nudge (0x0F). A-healthy + B-silent proves the nudge byte
// silences the child (VDISCARD/FLUSHO on BSD ttys). Fallback evidence
// (ps/lsof/sample) is collected only for a silent A.
// ---------------------------------------------------------------------------
#[cfg(target_os = "macos")]
#[tokio::test(flavor = "multi_thread")]
async fn probe_macos_nudge_silence_pair() {
    fn diag(app: &mut AppUnderTest, name: &str, report: &mut String) {
        match app.child.try_wait() {
            Ok(Some(status)) => {
                report.push_str(&format!("[{name}] EXITED, code={}\n", status.exit_code()));
            }
            Ok(None) => report.push_str(&format!("[{name}] still running\n")),
            Err(error) => report.push_str(&format!("[{name}] try_wait error: {error}\n")),
        }
        let Some(pid) = app.child.process_id() else {
            report.push_str(&format!("[{name}] no pid available\n"));
            return;
        };
        let ps = std::process::Command::new("ps")
            .args([
                "-o",
                "pid,ppid,pgid,sess,stat,etime,comm",
                "-p",
                &pid.to_string(),
            ])
            .output();
        if let Ok(out) = ps {
            ...
            report.push_str(&format!("[{name}] ps:\n{text}\n"));
            // Stopped ('T')? Try SIGCONT and note whether output resumes.
            if text
                .lines()
                .any(|l| l.split_whitespace().any(|t| t == "T" || t == "T+"))
            {
                ...
                report.push_str(&format!(
                    "[{name}] was STOPPED; after SIGCONT output: {:?}\n",
                    app.output()
                ));
            }
        }
        ...
        // Best-effort stack sample (may be permission-denied on runners).
        let file = std::env::temp_dir().join(format!("dsh-probe-sample-{pid}.txt"));
        if let Ok(_out) = std::process::Command::new("/usr/bin/sample")
            .args([
                pid.to_string(),
                "2".to_string(),
                "-file".to_string(),
        ...
        {
            let cut = text.len().min(6000);
            report.push_str(&format!("[{name}] sample:\n{}\n", &text[..cut]));
        }
    }

    let mut report = format!("macOS nudge probe (runner pid {})\n", std::process::id());
    for (key, value) in std::env::vars() {
        let key = key.to_ascii_lowercase();
        if key.contains("proxy") || key.starts_with("dsh") || key == "term" || key == "colorterm" {
            report.push_str(&format!("  env {key}={value}\n"));
        }
    }
    ...
    .await;
    mock.set_history("sA", &history_template("mA", "probe history"))
        .await;

    // Twin A: NO nudge. Twin B: the Scenario::boot nudge, byte for byte.
    let mut plain = AppUnderTest::spawn(mock.port(), 120, 30);
    let mut nudged = AppUnderTest::spawn(mock.port(), 120, 30);
    nudged.send(b"\x0f");

    let deadline = Instant::now() + Duration::from_secs(6);
    loop {
        if plain.output().contains("attached to") || Instant::now() > deadline {
            break;
        }
    ...
    let posts = mock
        ...
        .collect::<Vec<_>>();
    report.push_str(&format!(
        "[mock] captured POST paths: {posts:?}\n[A no-nudge] attached: {} output: {:?}\n[B nudged] attached: {} output: {:?}\n",
        plain.output().contains("attached to"),
        plain.output(),
        nudged.output().contains("attached to"),
        nudged.output(),
    ));

    let a_healthy = plain.output().contains("attached to");
    if !a_healthy {
        diag(&mut plain, "A", &mut report);
        diag(&mut nudged, "B", &mut report);
    }
    // Bonus datum: toggle discard OFF on B (only effective if B is still
    // canonical; recorded either way).
    let before = nudged.output();
    nudged.send(b"\x0f");
    std::thread::sleep(Duration::from_millis(1500));
    let after = nudged.output();
    report.push_str(&format!(
        "[B second 0x0F] output grew {} -> {} bytes; now: {:?}\n",
        before.len(),
        after.len(),
        after
    ));

    // Verdict: A must talk. B's state is a reported bonus datum (a second
    // toggle could legitimately resume a still-canonical child).
    assert!(
        a_healthy,
        "CHILD SILENT EVEN WITHOUT NUDGE — VDISCARD theory refuted; chase the report\n{report}"
    );
    eprintln!(
        "PROBE VERDICT: no-nudge twin healthy, nudged twin attached={} — VDISCARD/FLUSHO {}; report:\n{report}",
        after.contains("attached to"),
        if !before.contains("attached to") && !after.contains("attached to") {
            "CONFIRMED"
        } else {
            "REFUTED"
        }
    ...
    let _ = plain.quit(Duration::from_secs(2));
    let _ = nudged.quit(Duration::from_secs(2));
    mock.stop().await;
}
